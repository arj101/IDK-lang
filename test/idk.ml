...
let run_test prev_tests name test =
  let test_num =
    match Hashtbl.find_opt prev_tests name with
    | Some prev_num ->
        Hashtbl.replace prev_tests name (prev_num + 1);
        prev_num + 1
    | None ->
        Hashtbl.replace prev_tests name 0;
        0
  in
  Printf.printf ":: %s%s -> " name
    (if test_num > 0 then Printf.sprintf "_%d" test_num else "");
...
let test common_name (tests : (string * (unit -> bool)) list) =
  ...
  let succeeded =
    List.fold_left
      (fun acc (name, test) ->
        if run_test prev_tests name test then acc + 1 else acc)
      0 tests
  in
  let test_count = List.length tests in
  Printf.printf "\n%d/%d tests succeeded\n" succeeded test_count;
  if succeeded < test_count then raise (TestsFailed (test_count - succeeded))
  else ()
...
let () =
  test "Lexer"
    [
      ...
      ( "numbers",
        fun _ -> tokenised_eq "-123.0456789" [ Minus; Number 123.0456789 ] );
      ("numbers", fun _ -> tokenised_eq "-1.2e2" [ Minus; Number 1.2e2 ]);
      ( "single line comment",
        fun _ -> tokenised_eq "//**hides in the comment again**" [] );
      ( "single line comment",
        fun _ -> tokenised_eq "//*hides in the comment*\n;" [ Semicolon ] );
      ("string", fun _ -> tokenised_eq {|"string"|} [ Str "string" ]);
      ( "string",
        fun _ -> tokenised_eq {|"\tst\rring\n\0"|} [ Str "\tst\rring\n\0" ] );
      ( "string",
        fun _ -> tokenised_eq {|'\tst\rring\n\0'|} [ Str "\tst\rring\n\0" ] );
      ("string", fun _ -> tokenised_eq "\"st\nri\ng\"" [ Str "st\nri\ng" ]);
      ( "identifier",
        fun _ ->
          tokenised_eq "javascript > rust"
            [ Ident "javascript"; Greater; Ident "rust" ] );
    ]
